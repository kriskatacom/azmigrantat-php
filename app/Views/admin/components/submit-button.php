<?php
$text = $text ?? 'Запазване';
$class = $class ?? 'bg-primary-dark hover:bg-opacity-90';
$uid = uniqid();
?>

<div class="pt-5 border-t border-gray-100 flex gap-4">
    <button type="submit" id="submit-btn-<?= $uid ?>" class="<?= $class ?> text-white px-10 py-3 rounded-xl font-bold shadow-lg transition flex items-center justify-center min-w-40 disabled:opacity-70 disabled:cursor-not-allowed">
