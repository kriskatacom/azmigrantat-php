...
<div class="space-y-2">
    ...
    <select name="<?= $name ?>" <?= $attributes ?> class="form-control bg-white cursor-pointer focus:border-indigo-300">
        <option value=""><?= $placeholder ?></option>
        
        <?php foreach ($options as $parent): ?>
            <?php if ($excludeId && $parent['id'] == $excludeId) continue; ?>
            ...
            <?php if (!empty($parent['children'])): ?>
                <?php foreach ($parent['children'] as $child): ?>
                    <?php if ($excludeId && $child['id'] == $excludeId) continue; ?>
