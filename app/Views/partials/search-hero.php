...
$title = $title ?? HelperService::getTranslation($country, 'name', 'country');
$searchValue = $searchValue ?? false;
$showSearch = $showSearch ?? false;
?>

<section class="relative w-full h-150 md:h-130 lg:h-140 overflow-hidden group">
                    ...
                    <div class="w-full max-w-sm md:max-w-lg lg:max-w-xl px-4">
                        <form action="<?= $formAction ?>" method="GET" class="relative group/form">
                            <input type="text"
                                ...
                                class="w-full pl-4 pr-12 py-2 text-sm rounded-md md:py-2 md:pl-4 md:pr-20 md:text-xl md:rounded-lg bg-white/90 backdrop-blur-md text-gray-900 shadow-2xl focus:outline-none focus:ring-4 focus:ring-primary-light/50 transition-all placeholder:text-gray-500">
